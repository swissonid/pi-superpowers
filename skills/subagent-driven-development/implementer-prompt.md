...
Fill the placeholders and pass as the `task` parameter to `subagent({ agent: "worker", ... })`.
...
|---|---|
| `{N}` | Task number from plan |
| `{TASK_NAME}` | Task title |
| `{FULL_TASK_TEXT}` | Complete task text from plan (paste it, don't reference the file) |
| `{SCENE_SETTING_CONTEXT}` | Where this fits, dependencies, architectural context |
| `{WORKING_DIRECTORY}` | Directory to work from |
